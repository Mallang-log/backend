...
import static com.mallang.acceptance.post.PostAcceptanceSteps.포스트_단일_조회_요청을_보낸다;
import static com.mallang.acceptance.post.PostAcceptanceSteps.포스트_단일_조회_응답을_검증한다;
import static com.mallang.acceptance.post.PostAcceptanceSteps.포스트_생성_요청을_보낸다;
import static com.mallang.acceptance.post.PostAcceptanceSteps.포스트_수정_요청을_보낸다;
import static com.mallang.acceptance.post.PostAcceptanceSteps.포스트_전체_조회_요청을_보낸다;
...
public class PostAcceptanceTest extends AcceptanceTest {
    ...
    void 포스트를_작성한다() {
        ...
        // when
        var 응답 = 포스트_생성_요청을_보낸다(말랑_세션_ID, "첫 포스트", "첫 포스트이네요.", 카테고리_ID);
    ...
    void 포스트를_업데이트한다() {
        // given
        var 말랑_세션_ID = 회원가입과_로그인_후_세션_ID_반환("말랑");
        var 생성된_포스트_ID = ID를_추출한다(포스트_생성_요청을_보낸다(말랑_세션_ID, "첫 포스트", "첫 포스트이네요.", 없음()));
        var 카테고리_ID = ID를_추출한다(카테고리_생성_요청을_보낸다(말랑_세션_ID, "Spring", 없음()));

        // when
        var 응답 = 포스트_수정_요청을_보낸다(말랑_세션_ID, 생성된_포스트_ID, "업데이트 제목", "업데이트 내용", 카테고리_ID);

        // then
        응답_상태를_검증한다(응답, 정상_처리);
        var 예상_데이터 = 예상_포스트_단일_조회_응답(생성된_포스트_ID, "말랑", 카테고리_ID, "Spring", "업데이트 제목", "업데이트 내용");
        var 조회_결과 = 포스트_단일_조회_요청을_보낸다(생성된_포스트_ID);
    ...
    void 포스트_단일_조회() {
        ...
        var 말랑_세션_ID = 회원가입과_로그인_후_세션_ID_반환("말랑");
        var 카테고리_ID = ID를_추출한다(카테고리_생성_요청을_보낸다(말랑_세션_ID, "Spring", 없음()));
        var 생성된_포스트_ID = ID를_추출한다(포스트_생성_요청을_보낸다(말랑_세션_ID, "첫 포스트", "첫 포스트이네요.", 카테고리_ID));
        var 예상_데이터 = 예상_포스트_단일_조회_응답(생성된_포스트_ID, "말랑", 카테고리_ID, "Spring", "첫 포스트", "첫 포스트이네요.");

        // when
        var 응답 = 포스트_단일_조회_요청을_보낸다(생성된_포스트_ID);
    ...
    void 없는_포스트_단일_조회() {
        ...
        // when
        var 응답 = 포스트_단일_조회_요청을_보낸다(없는_ID);
    ...
    void 포스트_전체_조회() {
        ...
        var 말랑_세션_ID = 회원가입과_로그인_후_세션_ID_반환("말랑");
        var 카테고리_ID = ID를_추출한다(카테고리_생성_요청을_보낸다(말랑_세션_ID, "Spring", 없음()));
        var 포스트1_ID = ID를_추출한다(포스트_생성_요청을_보낸다(말랑_세션_ID, "포스트1", "이건 첫번째 포스트이네요.", 없음()));
        var 포스트2_ID = ID를_추출한다(포스트_생성_요청을_보낸다(말랑_세션_ID, "포스트2", "이번에는 이것 저것들에 대해 알아보아요", 카테고리_ID));
        var 포스트3_ID = ID를_추출한다(포스트_생성_요청을_보낸다(말랑_세션_ID, "포스트3", "잘 알아보았어요!", 카테고리_ID));
