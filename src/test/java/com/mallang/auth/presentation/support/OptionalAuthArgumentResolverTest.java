...
@DisplayName("선택적 인증 ArgumentResolver(OptionalAuthArgumentResolver) 은(는)")
@SuppressWarnings("NonAsciiCharacters")
@DisplayNameGeneration(ReplaceUnderscores.class)
class OptionalAuthArgumentResolverTest {

    private final AuthContext authContext = mock(AuthContext.class);
    private final OptionalAuthArgumentResolver authArgumentResolver = new OptionalAuthArgumentResolver(authContext);

    @SuppressWarnings({"rawtypes", "unchecked"})
    @Test
    void OptionalAuth_어노테이션과_Long_타입에_대해_동작한다() {
        // given
        MethodParameter methodParameter = mock(MethodParameter.class);
        given(methodParameter.hasParameterAnnotation(OptionalAuth.class))
                .willReturn(true);
    ...
    @SuppressWarnings({"rawtypes", "unchecked"})
    @Test
    void OptionalAuth_어노테이션이_없으면_동작하지_않는다() {
        // given
        MethodParameter methodParameter = mock(MethodParameter.class);
        given(methodParameter.hasParameterAnnotation(OptionalAuth.class))
                .willReturn(false);
    ...
    @Test
    void resolveArgument_시_저장된_인증정보가_없다면_null_을_반환한다() {
        // given
        given(authContext.unAuthenticated()).willReturn(true);
